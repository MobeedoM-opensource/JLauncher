...
public class OverviewSettingsPanel {
    ...
    public void initializeAdapter() {
        ...
        String[] headers_full = new String[]{
                res.getString(R.string.home_screen_settings),
                res.getString(R.string.drawer_settings),
                res.getString(R.string.app_settings)};
        String[] headers_nodrawer = new String[]{
                res.getString(R.string.home_screen_settings),
                res.getString(R.string.app_settings)};

        String[] headers = mLauncher.isUsingExternalDrawer() ? headers_nodrawer:headers_full;
        ...
        if(!mLauncher.isUsingExternalDrawer())
            mSettingsAdapter.addPartition(false, true);
        mSettingsAdapter.addPartition(false, true);
        mSettingsAdapter.mPinnedHeaderCount = headers.length;

        int idx = 0;
        mSettingsAdapter.changeCursor(idx, // HOME_SETTINGS_POSITION
                createCursor(headers[idx++], getValuesHome()));
        if(!mLauncher.isUsingExternalDrawer())
            mSettingsAdapter.changeCursor(idx, // DRAWER_SETTINGS_POSITION
                    createCursor(headers[idx++], getValuesDrawer()));
        mSettingsAdapter.changeCursor(idx, // APP_SETTINGS_POSITION
                createCursor(headers[idx++], valuesApp));
    ...
    private String[] getValuesHome() {
        ...
        ArrayList<String> values = new ArrayList<String>(Arrays.asList(new String[]{
                res.getString(R.string.external_drawer),
