...
import com.android.launcher3.extras.Constants;
import com.android.launcher3.extras.IAidlDrawerProviderIntf;
import com.android.launcher3.extras.RemoteExecReceiver;
import com.android.launcher3.extras.Utils;
...
import static com.android.launcher3.extras.Constants.DEFDRAWER_ID;
import static com.android.launcher3.extras.Constants.DEFDRAWER_MIN_V;
...
public class Launcher extends Activity
    ...
    private static final int REQUEST_CHOOSE_DRAWER = 200;
    ...
    static final String INTENT_EXTRA_IGNORE_LAUNCH_ANIMATION = BuildConfig.APPLICATION_ID + ".intent.extra.shortcut.INGORE_LAUNCH_ANIMATION";
    ...
    static final String ACTION_FIRST_LOAD_COMPLETE = BuildConfig.APPLICATION_ID + ".action.FIRST_LOAD_COMPLETE";
    ...
    private final BroadcastReceiver mRemoteControlReceiver = new RemoteExecReceiver();
    private boolean mUseExternalDrawer = false;
    protected IAidlDrawerProviderIntf mDrawerProviderService;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        registerReceiver(mRemoteControlReceiver, new IntentFilter(Constants.REMOTE_EXEC_INTENT));
        ...
        Constants.readRemoteCfg(this);
        bindToDrawerProviderService();
    ...
    public void onSettingsChanged(String settings, boolean value) {
        ...
        if (SettingsProvider.SETTINGS_UI_USE_EXTERNAL_DRAWER.equals(settings)) {
            mUseExternalDrawer = value;
        }
    }
    ...
    private void handleActivityResult(
        ...
        } else if (requestCode == REQUEST_CHOOSE_DRAWER) {
            if (resultCode == RESULT_OK) {
                saveExternalDrawerUri(data.toUri(0));
                notifyExternalDrawerChanged();
            } else if (getExternalDrawerUri(this) == null || !Utils.isAppInstalled(this, getExternalDrawerUri(this), DEFDRAWER_MIN_V)) {
                // no previous saved drawer, switching off
                SettingsProvider.putBoolean(this, SettingsProvider.SETTINGS_UI_USE_EXTERNAL_DRAWER, false);
                if(getExternalDrawerUri(this) != null && Utils.isAppInstalled(this, getExternalDrawerUri(this)))
                    Toast.makeText(Launcher.this, "External drawer set but not up to date: please update it to the latest version.\nSwitching back to the default drawer.", Toast.LENGTH_LONG).show();
                else
                    Toast.makeText(Launcher.this, "No external drawer set, switching back to the default drawer.", Toast.LENGTH_LONG).show();
                onUseExternalDrawerChanged();
            }
        }
    ...
    public void initializeDeviceProfile(LauncherAppState app) {
        ...
        mUseExternalDrawer = SettingsProvider.getBoolean(this,
                SettingsProvider.SETTINGS_UI_USE_EXTERNAL_DRAWER,
                R.bool.preferences_interface_use_external_drawer);
    ...
    public boolean isUsingExternalDrawer() {
        return mUseExternalDrawer;
    }

    public static boolean isExternalDrawerActive(Context context) {
        return SettingsProvider.getBoolean(context, SettingsProvider.SETTINGS_UI_USE_EXTERNAL_DRAWER,
                R.bool.preferences_interface_use_external_drawer);
    }

    public void onUseExternalDrawerChanged() {
        boolean oldValue = mUseExternalDrawer;
        mUseExternalDrawer = isExternalDrawerActive(this);

        if (mUseExternalDrawer) {
            if(Constants.isDefDrawerAvailable)
                showDrawerChooser();
            else
                showGetDrawerDialog();
        } else
        ...
        if (mUseExternalDrawer != oldValue) {
            mOverviewSettingsPanel.initializeAdapter();
            mOverviewSettingsPanel.notifyDataSetInvalidated();
        }
    }

    private void notifyExternalDrawerChanged() {
        bindToDrawerProviderService();
    }

    private void showDrawerChooser() {
        Uri adress = Uri.parse(URL);
        Intent intent = new Intent(Constants.DRAWER_PROVIDER_INTENT);

        // TODO - Create a chooser intent, for choosing which Activity
        // will carry out the baseIntent. Store the Intent in the
        // chooserIntent variable below. HINT: using the Intent class'
        // createChooser())
        Intent chooserIntent = Intent.createChooser(intent, getString(R.string.choose_drawer));
        //chooserIntent = null;

        Log.i(TAG, "Chooser Intent Action:" + chooserIntent.getAction());
        // TODO - Start the chooser Activity, using the chooser intent
        if (true || intent.resolveActivity(getPackageManager()) != null) {
            startActivityForResult(chooserIntent, REQUEST_CHOOSE_DRAWER);
        }
    }

    protected void showGetDrawerDialog() {
        if(mUseExternalDrawer && !Constants.isDefDrawerAvailable && DEFDRAWER_ID != null) {
            new AlertDialog.Builder(this)
                    .setTitle(Constants.isDefDrawerOutdated ? R.string.ext_drawer_not_updated : R.string.ext_drawer_not_found)
                    .setMessage(Constants.isDefDrawerOutdated ? R.string.ext_drawer_not_updated_msg : R.string.ext_drawer_not_found_msg)
                    .setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            try {
                                startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=" + DEFDRAWER_ID)));
                            } catch (ActivityNotFoundException e) {
                                try {
                                    startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("https://play.google.com/store/apps/details?id=" + DEFDRAWER_ID)));
                                } catch (Exception e1) {
                                    Toast.makeText(Launcher.this, "Unable to connect to the playstore", Toast.LENGTH_SHORT).show();
    ...
    public Intent getExternalDrawerIntent() {
        Intent i = null;
        try {
            String drawerStr = getExternalDrawerUri(this);
            if (drawerStr != null)
                i = Intent.parseUri(drawerStr, 0);
        } catch (Throwable e) {
            e.printStackTrace();
        }
        return i;
    ...
    public static String getExternalDrawerUri(Context context) {
        return SettingsProvider.getStringCustomDefault(context, SettingsProvider.SETTINGS_EXTERNAL_DRAWER_URI, null);
    }

    public String getExternalDrawerPackage() {
        Intent i = getExternalDrawerIntent();
        if (i != null) {
    ...
    private void saveExternalDrawerUri(String uri) {
        if(uri != null)
            SettingsProvider.putString(this, SettingsProvider.SETTINGS_EXTERNAL_DRAWER_URI, uri);
        else {
            SettingsProvider.deleteKey(this, SettingsProvider.SETTINGS_EXTERNAL_DRAWER_URI);
            SettingsProvider.putBoolean(this, SettingsProvider.SETTINGS_UI_USE_EXTERNAL_DRAWER, false);
            onUseExternalDrawerChanged();
        }
    }
    ...
    public void setLauncherEffect(BaseEffectAnimation.Effect effect) {
        int lastEffectType = SettingsProvider.getIntCustomDefault(this,
                SettingsProvider.SETTINGS_UI_WORKSPACE_EFFECT, 5);
    ...
    private boolean showAppsOrWidgets(State toState, boolean animated, boolean focusSearchBar) {
        ...
        if (toState == State.APPS) {
            Intent i = null;
            if (mUseExternalDrawer && (i = getExternalDrawerIntent()) != null) {
                try {
                    startActivity(i);
                    return true;
                } catch (Exception e) {
                    Log.e(TAG, "Error in showAppsOrWidgets", e);
                    Toast.makeText(this, "There was a problem starting the external drawer, switching back to the default", Toast.LENGTH_SHORT).show();
                    if(!Utils.isAppInstalled(this, getExternalDrawerPackage())) {
                        // cleanup
                        saveExternalDrawerUri(null);
                    }
                }
            }
    ...
    private void bindToDrawerProviderService() {
        if (mConnection != null) {
            try {
                if (mDrawerProviderService != null)
                    mDrawerProviderService.onDrawerDisabled();
                try {
                    unbindService(mConnection);
                } catch (Exception e) { }
            } catch (RemoteException e) {
                Log.e(TAG, "Error in bindToDrawerProviderService", e);
            }
        }

        // the first time force the default drawer if present
        if(getExternalDrawerPackage() != null) {
            Intent i = new Intent(IAidlDrawerProviderIntf.class.getName());
            i.setAction(Constants.DRAWER_PROVIDER_SERVICE);
            i.setPackage(getExternalDrawerPackage());
            bindService(i, mConnection, Context.BIND_AUTO_CREATE);
            Log.d(TAG, String.format("Launcher.bindToDrawerProviderService: %s", getExternalDrawerPackage()));
        } else {
            Log.d(TAG, String.format("Launcher.bindToDrawerProviderService: SKIPPED no extdrawer available"));
        }
    }
    ...
    private ServiceConnection mConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName componentName, IBinder iBinder) {
            mDrawerProviderService = IAidlDrawerProviderIntf.Stub.asInterface(iBinder);
            try {
                if (mUseExternalDrawer) {
                    saveExternalDrawerUri(mDrawerProviderService.onDrawerEnabled());
                } else
                    mDrawerProviderService.onDrawerDisabled();
            } catch (RemoteException e) {
                Log.e(TAG, "Error in onServiceConnected", e);
            }
            Log.d(TAG, String.format("Launcher.onServiceConnected: "));
        }

        @Override
        public void onServiceDisconnected(ComponentName componentName) {
            Log.d(TAG, String.format("Launcher.onServiceDisconnected: "));
            mDrawerProviderService = null;
        }
    };
...
interface DebugIntents {
    static final String DELETE_DATABASE = BuildConfig.APPLICATION_ID + ".action.DELETE_DATABASE";
    static final String MIGRATE_DATABASE = BuildConfig.APPLICATION_ID + ".action.MIGRATE_DATABASE";
}
